...
func TestUpdate(t *testing.T) {
	...
	args = Arguments{HTTPAddress: "localhost", HTTPPort: 8921, ForwardTo: testAppendable(actualSamples)}
...
func TestServerRestarts(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			initialArgs:   Arguments{HTTPAddress: "localhost", HTTPPort: 8922, ForwardTo: []storage.Appendable{}},
			newArgs:       Arguments{HTTPAddress: "localhost", HTTPPort: 8922, ForwardTo: []storage.Appendable{}},
			shouldRestart: false,
		},
		{
			name:          "forward_to update does not require restart",
			initialArgs:   Arguments{HTTPAddress: "localhost", HTTPPort: 8922, ForwardTo: []storage.Appendable{}},
			newArgs:       Arguments{HTTPAddress: "localhost", HTTPPort: 8922, ForwardTo: testAppendable(nil)},
			shouldRestart: false,
		},
		{
			name:          "hostname change requires restart",
			initialArgs:   Arguments{HTTPAddress: "localhost", HTTPPort: 8922, ForwardTo: []storage.Appendable{}},
			newArgs:       Arguments{HTTPAddress: "127.0.0.1", HTTPPort: 8922, ForwardTo: testAppendable(nil)},
			shouldRestart: true,
		},
		{
			name:          "port change requires restart",
			initialArgs:   Arguments{HTTPAddress: "localhost", HTTPPort: 8922, ForwardTo: []storage.Appendable{}},
			newArgs:       Arguments{HTTPAddress: "localhost", HTTPPort: 8923, ForwardTo: testAppendable(nil)},
			shouldRestart: true,
		},
	}
...
func waitForServerToBeReady(t *testing.T, comp *Component) {
	require.Eventuallyf(t, func() bool {
		resp, err := http.Get(fmt.Sprintf("http://%v:%d/wrong/path", comp.args.HTTPAddress, comp.args.HTTPPort))
...
func verifyExpectations(
	...
	endpoint := fmt.Sprintf("http://%s:%d/api/v1/metrics/write", args.HTTPAddress, args.HTTPPort)
